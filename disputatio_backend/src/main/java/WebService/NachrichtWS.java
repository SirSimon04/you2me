...
public class NachrichtWS{
     ...
     * @param token Das Webtoken
     * @return Die angefragte Nachricht
     */
    @GET
    @Path("/{id}/{token}")
    @Produces(MediaType.APPLICATION_JSON)
    ...
    public Response getByChat(@PathParam("id") int chatid, @PathParam("token") String token, @PathParam("nutzerid") int nutzerid) throws IOException{
        if(!verify(token)){
            ...
            if(settingsEJB.getById(self.getId()).getWordfilter()){
                for(Nachricht n : nList){
     ...
     */
    @GET
    @Path("/chat/getNewest/{id}/{nutzerid}/{token}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getNewest(@PathParam("id") int id,
        ...
        if(!verify(token)){
            ...
            Nutzer self = nutzerEJB.getById(nutzerid);
            if(self.getMarkedMessages().contains(n)){
                n.setIsMarked(true);
            }
            return response.generateAnswer(parser.toJson(n));
        }
    ...
    public Response getInfo(@PathParam("id") int id,
        ...
        if(!verify(token)){
            ...
            Nutzer self = nutzerEJB.getCopyById(nutzerid);
            ...
            gelesenVon.remove(self);
            gelesenVon.add(0, self);
...
//                return response.generateAnswer(parser.toJson(nachrichtEJB.getCopyById(id)));
    ...
    /**
     * Diese Methode gibt den Channelfeed eines Nutzers zurück. DIeser beinhaltet alle
     * Posts, die von Freunden eines Nutzers verfasst worden sind.
     *
     * @param id Die Id des Nutzers
     * @param token Das Webtoken
     * @return Die Posts des Channels
     */
    @GET
    @Path("/getChannelFeed/{id}/{token}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getChannelFeed(@PathParam("id") int id,
            @PathParam("token") String token
    ){
        if(!verify(token)){
            return response.generateError401("Kein gültiges Token");
            ...
            List<Nutzer> friendList = nutzerEJB.getFriends(id);
            List<Nachricht> posts = new ArrayList<>();
            for(Nutzer n : friendList){
                posts.addAll(nachrichtEJB.getCopyByChat(n.getChannel().getChatid()));
            }
            List<JsonObject> returnList = new ArrayList<JsonObject>();
            Date now = new Date(System.currentTimeMillis());
            if(posts.size() > 1){
                Collections.sort(posts, (Nachricht z1, Nachricht z2) -> {
                    if(z1.getDatumuhrzeit() > z2.getDatumuhrzeit()){
                        return -1;
                    }
                    if(z1.getDatumuhrzeit() < z2.getDatumuhrzeit()){
            ...
            for(Nachricht n : posts){
                ...
                if(n.getLikedBy().contains(self)){
                    j.add("likedbyme", parser.toJsonTree(true));
                }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response send(String Daten,
            @PathParam("token") String token) throws IOException, MessagingException, AddressException, InterruptedException{
            ...
            try{
                ...
                if(!c.getIsgroup()){
                    ...
                    if(self.getHatBlockiert().contains(other)){
                        return response.generateError406(parser.toJson("youblocked"));
                    }else if(other.getHatBlockiert().contains(self)){
                        return response.generateError406(parser.toJson("gotblocked"));
                    }
                }
                            ...
                            if(s.getMailifimportant()){
                                ...
                                if(cc.getIsgroup()){
                                    msg += "<h2>Hallo " + n.getBenutzername() + ",</h2>"
                                            ...
                                            + "\"</p>"
                                            + "<p>Falls du keine Benachrichtigungen mehr erhalten willst, "
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.APPLICATION_JSON)
    public Response editMessage(String Daten,
            @PathParam("token") String token) throws IOException, MessagingException, AddressException, InterruptedException{
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.APPLICATION_JSON)
    public Response sendFile(String Daten,
            @PathParam("token") String token
    ){
        if(!verify(token)){
            return response.generateError401("Kein gültiges Token");
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.APPLICATION_JSON)
    public Response delete(String Daten,
            @PathParam("token") String token
    ){
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.APPLICATION_JSON)
    public Response markMessage(String Daten,
            @PathParam("token") String token
    ){
        if(!verify(token)){
            return response.generateError401("Kein gültiges Token");
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getMarked(@PathParam("id") int id,
            @PathParam("token") String token
    ){
        if(!verify(token)){
            return response.generateError401("Kein gültiges Token");
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.APPLICATION_JSON)
    public Response sendInChannel(String Daten,
            @PathParam("token") String token) throws IOException, MessagingException, AddressException, InterruptedException{
        ...
        if(!verify(token)){
            ...
            try{
                ...
                neueNachricht.setSenderid(eigeneId);
                nachrichtEJB.add(neueNachricht);
    ...
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.APPLICATION_JSON)
    public Response like(String Daten,
            @PathParam("token") String token
    ){
        if(!verify(token)){
            return response.generateError401("Kein gültiges Token");
